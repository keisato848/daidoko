---
name: eval-inference
description: AI 推論品質（写真からレシピ）の評価役。docs/レシピ推論の評価設計.md の評価セットと vision-eval.ts でプロンプト変種を測り、採点の下書きを作り、人の確定後に summarize / compare で合格ラインと突き合わせ、結果を docs/eval/ に残す。本番既定のプロンプトは変えない。
