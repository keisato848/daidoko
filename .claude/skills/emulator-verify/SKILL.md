...
広告ユニット無しのビルドは広告視聴を飛ばして直行するので、AI 経路（相談・写真からレシピ）の検証は
`EXPO_PUBLIC_FREE_DAILY_LIMIT=999` を付けて組む（2026-09-15・AQUOS で被弾。§4-0b の「枠切れ」の別パターン）。
worktree からビルドするなら先に `docs/開発ハーネス.md` §4-0c を読む。
...
- 状態を確定させたいときは事前に `adb shell am force-stop com.daidoko.app`（コールドスタート）。
  ただし**コールドスタート直後の deep link はホーム画面に差し替えられることがある**
  （2026-09-09）— アプリが一度温まってから送ると安定する
...
| `set-app-locales ja-JP` 後に `input text` がかな合成される                                                               | Gboard もアプリと一緒に日本語 IME へ切り替わる → `keyevent 204`（LANGUAGE_SWITCH）で英語配列へ。1 文字目が落ちることがあるので dump で確認（詳細: 開発ハーネス §4-0）                                                                                                                                                                    |
| 1280 幅 AVD の縮小スクショから座標を出すとズレる                                                                         | `device-shot.ps1` の縮小率は解像度依存で ×2 とは限らない（1280 幅では実測 ×2.37）→ 座標は `uiautomator dump` の bounds から取る                                                                                                                                                                                                          |
| `compileReleaseKotlin` が `Module was compiled with an incompatible version of Kotlin`（play-services-ads 25.x）で落ちる | この PC の `android/`（gitignore）が古く、app.json の `kotlinVersion: 2.2.20` と `withKotlinMetadataSkip` が反映されていない → `--prebuild` を付けて組み直す（2026-09-11・リモートセッションで初ビルド時）                                                                                                                               |
| prebuild 後に `ReactNativeApplicationEntryPoint.java: パッケージ com.app は存在しません`                                 | 古い `android/` で走った 1 回目が `android/build/generated/autolinking/` に `packageName: com.app` を残し、package.json が同じなので再利用される → そのディレクトリを消して組み直す（2026-09-11）                                                                                                                                        |
| リモートセッション（VS Code 拡張経由）から起動したエミュレータが `Showing crashdialog` で止まる                          | ホスト GPU で qemu が落ちてクラッシュ同意ダイアログを待つ → `-no-window -no-metrics -gpu swiftshader_indirect` を付けて起動する。screencap は動く（2026-09-11）                                                                                                                                                                          |
| アプリデータの消去・アンインストールが hook で止まり、初回起動をやり直せない                                             | 実機保護のガード（`hook-pretool-guard.mjs`）。エミュレータでも同じ → `adb emu kill` → 同じ AVD を `-wipe-data` で起動し直す（アプリごと消える）                                                                                                                                                                                          |
| サンプルデータのレシピ名・タグが英語で入る                                                                               | シードは**初回起動時の言語**で入る。エミュレータ既定が en-US のため → インストール直後・初回起動前に `cmd locale set-app-locales com.daidoko.app --user 0 --locales ja-JP`                                                                                                                                                               |
| コーチマーク表示中は座標タップが全部吸われ、次の画面に進まない                                                           | `uiautomator dump` の content-desc `次のガイドへ` / `ガイドを閉じる` / `ガイドをスキップ` の bounds を取ってタップする。座標の決め打ちは 2 枚目で位置が変わるので使えない                                                                                                                                                                |
