# だいどこ — CLAUDE.md

> Claude Code が毎回読むプロジェクト憲法。実装前に必ずこのファイル全体を読むこと。

---

## 1. プロダクト概要

**アプリ名:** だいどこ（臺所）  
**コンセプト:** 家族で育てる、台所のレシピ手帳  
**プラットフォーム:** iOS / Android（React Native + Expo）、将来的に Web  
**ステージ:** 設計完了・実装開始前

---

## 2. 設計書インデックス（実装前に必ず参照）

| ファイル                           | 内容                                                                        | 実装時の参照タイミング                     |
| ---------------------------------- | --------------------------------------------------------------------------- | ------------------------------------------ |
| `docs/要件定義.md`                 | 機能要件・非機能要件・ユーザーストーリー                                    | 機能実装前                                 |
| `docs/データ設計.md`               | 全 17 エンティティの ER 図・型定義・FTS5 設計                               | DB スキーマ・クエリ実装前                  |
| `docs/画面設計.md`                 | 全画面の UI 仕様・コンポーネント対応表                                      | 画面実装前                                 |
| `docs/アーキテクチャ設計.md`       | 技術スタック・同期設計・API エンドポイント一覧                              | 構成変更・新機能追加前                     |
| `docs/レシピ作成フロー.md`         | URL取り込み・OCR・手動入力のロジック詳細                                    | A1/A2/A4/A5 実装前                         |
| `docs/お店の味を再現設計.md`       | 主役機能候補。食べた→作った→近づける の再現ループ・データ拡張               | 再現ループ関連の実装前                     |
| `docs/相談してレシピを作る設計.md` | 蔵書庫への2本目の入口。まだ料理が無いときに会話で下書きを作る               | 相談機能の変更前                           |
| `docs/AIに聞く設計.md`             | アプリ内アシスタント（使い方を聞く→機能を開く）。カタログ・常駐ボタン・段階 | 使い方導線・HelpButton・アシスタント変更前 |
| `docs/レシピ推論の評価設計.md`     | 推論品質の採点軸・合格ライン・評価セット・プロンプト v1                     | プロンプト変更・主役交代の判断前           |
| `docs/フリーミアム設計.md`         | AI写真レシピの無料枠・課金(RevenueCat)・ゲート設計                          | 課金/写真レシピ変更前                      |
| `docs/買い物リスト・在庫設計.md`   | 在庫⇄買い物リスト⇄レシピのループ・データモデル・段階計画                    | 買い物/在庫機能 実装前                     |
| `docs/冷蔵庫写真設計.md`           | 冷蔵庫写真→在庫登録→作れるレシピの 1 フロー・確認シート・API                | 冷蔵庫写真機能の変更前                     |
| `docs/共有設計.md`                 | 共有 UX の全体設計。相手中心の再編・統一共有シート・用語統一                | 共有導線・文言の変更前                     |
| `docs/ウィジェット設計.md`         | 両 OS ウィジェット（スナップショット契約・工数・リスク）                    | ウィジェット実装前                         |
| `docs/レシピ表紙AI生成設計.md`     | 表紙の AI 画像生成（モデル・原価・ラベル・ゲート）                          | 表紙生成 実装前                            |
| `docs/クラウド同期設計.md`         | v2.0 アカウントレス家族同期・Expo Push・コスト/工数見積                     | 同期機能（#65）実装前                      |
| `docs/マルチエージェント設計.md`   | A1〜A8 エージェントの責務・インターフェース                                 | 各エージェント実装前                       |
| `docs/エージェントフック設計.md`   | AgentBridge・Hook システム・ログファイル設計                                | Bridge 実装前                              |
| `docs/品質基準.md`                 | テストカバレッジ閾値・CI ゲート・パフォーマンス目標値                       | PR 作成前に必ず確認                        |
| `docs/リリース手順.md`             | Railway デプロイ・EAS ビルド・Play CLI 提出・掲載更新                       | リリース作業前                             |
| `docs/開発ハーネス.md`             | Skill・エージェント・フック・実機検証規約の全体マップ                       | ハーネス変更・実機検証前                   |
| `docs/growth/流入の記録.md`        | DL 数・広告・ストア外施策の判断ログと閾値。ユーザー作業の一覧               | 広告費・掲載改修・流入施策の判断前         |
| `docs/brand/ロゴ仕様.md`           | カラー・フォント・ロゴ使用ルール                                            | UI 実装時                                  |
| `mockup/app-mockup.jsx`            | インタラクティブモックアップ（React JSX）                                   | 画面実装の参考                             |

---

## 3. 技術スタック

### モノレポ構成

```
daidoko/
├── apps/
│   ├── mobile/          # Expo SDK 54 + Expo Router v6
│   └── server/          # Hono + Node.js 20 + PostgreSQL 16
└── packages/
    └── shared/          # 共通型・Zod スキーマ・定数
```

パッケージマネージャー: **pnpm** (`pnpm-workspace.yaml` 参照)

### Mobile（apps/mobile）

| 役割             | ライブラリ                                 |
| ---------------- | ------------------------------------------ |
| フレームワーク   | Expo SDK 54                                |
| ナビゲーション   | Expo Router v6（ファイルベース）           |
| ローカル DB      | expo-sqlite + Drizzle ORM                  |
| サーバー状態     | TanStack Query v5                          |
| クライアント状態 | Zustand                                    |
| フォーム         | React Hook Form + Zod                      |
| カメラ / 画像    | expo-image-picker + expo-image-manipulator |
| アニメーション   | Reanimated 3                               |
| アイコン         | Lucide React Native                        |

### Server（apps/server）

| 役割           | ライブラリ                                    |
| -------------- | --------------------------------------------- |
| フレームワーク | Hono v4                                       |
| ランタイム     | Node.js 20 LTS                                |
| DB             | PostgreSQL 16 + Drizzle ORM                   |
| 認証           | JWT（jose）RS256、リフレッシュトークン Rotate |
| ログ           | pino + pino-roll（日次ローテーション）        |
| テスト         | Vitest + Supertest                            |

### 共通（packages/shared）

- Zod スキーマ（API リクエスト/レスポンス型）
- エージェント共通型（`AgentResult<T>`、`RecipeDraft` 等）
- AgentBridge・HookLogger

---

## 4. ブランドルール（UI 実装時に厳守）

| 用途                           | 表記                                             | フォント                    |
| ------------------------------ | ------------------------------------------------ | --------------------------- |
| アプリアイコン・スプラッシュ   | **湯気の立つ椀＋きらめき**（絵。文字は入れない） | —                           |
| ロゴ（掲載バナー等の日本語面） | **臺所**（正字体）                               | Shippori Mincho / Yu Mincho |
| アプリ内 UI テキスト全般       | **DAIDOKO**                                      | Cormorant Garamond Italic   |
| マーケティング・ストア説明     | **だいどこ**                                     | —                           |

カラーパレット:

```
背景:         #0A0805
ゴールド（主）: #C9A16A
テキスト:      #DCC9A8
罫線:         #2E2418
```

---

## 5. 開発ルール

### コード品質（`docs/品質基準.md` §2 参照）

- TypeScript `strict: true` 必須。`any` 禁止。
- ESLint エラー 0・Prettier 差分 0 でなければ PR 不可。
- テストカバレッジ: サービス層 ≥80%、ユーティリティ ≥90%、UI ≥60%。

### セキュリティ（`docs/品質基準.md` §1 参照）

- GitHub Security の Critical/High アラートはゼロトレランス（1件でも CI ブロック）。
- SQL は必ず Drizzle ORM または `sql` タグテンプレート経由。文字列結合 SQL 禁止。
- JWT は RS256（非対称鍵）のみ。HS256 禁止。

### エージェント実装ルール（`docs/マルチエージェント設計.md` 参照）

- エージェント間の呼び出しは必ず `AgentBridge.call()` 経由。直接インスタンスを呼ばない。
- 各エージェントは `before*/after*` フックを発火させる（`docs/エージェントフック設計.md` 参照）。
- エラー型は `AgentErrorCode` の共通コードを使用する。

### Git ルール

- ブランチ: `develop` ベース。機能ブランチは `feat/xxx`、修正は `fix/xxx`。
- コミットメッセージ: Conventional Commits（`feat:`, `fix:`, `test:`, `docs:`, `chore:`）。
- `main` への直接 push 禁止。PR 経由のみ。
- **マージ前に必ずエミュレーター/実機で動作確認する**（`.claude/settings.json` で配線された PreToolUse フック `scripts/agent/hook-pretool-guard.mjs` が `gh pr merge` / `git merge` を検知して確認を促す。全ルールは `docs/開発ハーネス.md` §2）。
- **リリース（ストア提出）は実機検証の記録が無いとできない。** エミュレーターだけでは不足（実データの規模・同名材料・明るい写真といった条件が揃わず、1.4.5 の準備では実機のみで4件の不具合が出た）。`node scripts/agent/record-device-verification.mjs --checked "…"` で記録してから提出する。記録後に `apps/` を変更したら再検証が必要（`docs/開発ハーネス.md` §4-2）。
- pre-commit はシークレットスキャン→Prettier 自動整形→スライス検証の順で走る（`docs/開発ハーネス.md` §3）。

### エージェントへの委譲（名指しが無くても呼ぶ）

`.claude/agents/` の役は、ユーザーが名指ししなくても**依頼の型で**呼ぶ。編成と権限は `docs/開発ハーネス.md` §7。
`UserPromptSubmit` フック（`scripts/agent/hook-user-prompt-router.mjs`）が同じ対応表で案内を注入するが、
案内が無くても以下に当たれば呼ぶ。束ねる役は配下の専門役を自分で呼ぶので、司令塔は束ねる役だけ呼べばよい。

| 依頼の型                                                       | 呼ぶ役                                                                                                                                                       |
| -------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 「今どこ」「次に何を」「残タスク」「出せる状態？」             | `project-manager`（セッション開始時も）                                                                                                                      |
| 「PR に出していい」「マージしていい」「テスト足りてる」        | `quality-manager`                                                                                                                                            |
| 「〜を作りたい」「どう思う」「優先度は」「訴求は」             | `product-manager`                                                                                                                                            |
| 「どう実装する」「どこを変える」「設計はこれでいい」           | `app-leader`                                                                                                                                                 |
| 「記録して」「覚えておいて」「ハマった」                       | `finding-recorder`                                                                                                                                           |
| サーバー・Railway・デプロイ・疎通                              | `server-verifier`                                                                                                                                            |
| リリースノート・新機能の文                                     | `release-notes-drafter`                                                                                                                                      |
| Issue・ボードの整理                                            | `issue-groomer`                                                                                                                                              |
| 「数字はどう」「継続率は」「広告の効果は」「収益化は効いてる」 | `growth-analyst`（数字が無ければ「無い」と言う）                                                                                                             |
| プロンプト変更・推論の精度                                     | `eval-inference`                                                                                                                                             |
| 実装を終えて PR を出す前                                       | `test-writer` → `quality-manager`（配下で diff-critic ＋ audit）→ `drift-audit`。重い変更は司令塔が `Workflow change-review` を先に回し、report を QM に渡す |
| 「エージェント構成で足りてる？」「スイートを見直したい」       | Skill `harness-audit`（7 軸監査）                                                                                                                            |

最終承認はユーザー。束ねる役の判定は推奨であり、司令塔は「判断・根拠・反対材料」を要約して伝える。

### 分かったことは必ずリポジトリに残す

調査・レビュー・実機検証で分かったことを、その場の説明で終わらせない。
**次に触る人（＝次のセッション）が同じ穴を掘り直さない形**でリポジトリに書く。
会話の中だけ・セッションのメモだけに置かない — どちらも次の作業者からは見えない。

| 分かったこと                               | 置き場所                                                      |
| ------------------------------------------ | ------------------------------------------------------------- |
| 仕様・設計の判断と、その理由               | 該当する `docs/*設計.md`（§2 の索引にある文書）               |
| 「設計書どおりに書いたら壊れた」の類       | 同じ設計書に**なぜ変えたか**込みで（例: クラウド同期 §5-1b）  |
| テストの書き方の罠（緑のまま本番が死ぬ類） | `docs/品質基準.md` §2.3                                       |
| 実機・エミュレーター操作の罠               | `docs/開発ハーネス.md` §4 と `.claude/skills/emulator-verify` |
| リリース手順・ストア運用でハマったこと     | `docs/リリース手順.md` / 該当 Skill                           |
| 未着手だが忘れてはいけないこと             | GitHub Issues（`docs/開発ハーネス.md` §7-2 が正）             |
| コードを読めば分かること                   | **書かない**（重複は必ず腐る）                                |

書くのは**症状 → 原因 → 対処**の順。「何分溶かしたか」「どの版で踏んだか」まで書くと、
次の人がその記述を信用してよいか判断できる。

---

## 6. よく使うコマンド

```bash
# 開発環境起動
docker compose -f docker-compose.dev.yml up -d   # PostgreSQL + MinIO
pnpm dev:mobile                                   # Expo 起動
pnpm dev:server                                   # Hono サーバー起動

# DB
pnpm --filter server db:generate   # Drizzle マイグレーションファイル生成
pnpm --filter server db:migrate    # マイグレーション適用
pnpm --filter server db:studio     # Drizzle Studio（GUI）

# 品質チェック
pnpm typecheck                     # 全パッケージ型チェック
pnpm lint                          # 全パッケージ ESLint
pnpm format:check                  # Prettier チェック
pnpm test                          # 全パッケージテスト
```

---

## 7. 実装フェーズ

| フェーズ               | 内容                                                                             | 目安   |
| ---------------------- | -------------------------------------------------------------------------------- | ------ |
| **v0.1 Alpha（現在）** | ホーム・蔵書・レシピ詳細の UI（ローカル固定データ）                              | 2 週   |
| v0.5 Beta              | 全画面 + SQLite DB 接続 + 料理中モード                                           | 6 週   |
| v1.0                   | URL 取り込み + 「作った！」記録 + 家族共有                                       | 10 週  |
| v1.5                   | OCR + ギャラリー + カレンダービュー                                              | 14 週  |
| v2.0                   | クラウド同期（家族共有・**2026-08-21 着手** — S0+S1+S2+#198 一括）+ Web 版は別途 | 実装中 |

---

## 8. v0.1 Alpha の定義（最初のセッションのスコープ）

以下のみを実装する。それ以外は実装しない。

### 対象画面（`docs/画面設計.md` 参照）

- S01: ホーム画面（家族の最新調理カード・作りたいリスト・月の統計）
- S04: レシピ一覧画面（検索 + フィルタタブ。食材名検索対応）
- S05: レシピ詳細画面（材料・手順・調理記録ボタン）
- S06: 料理中モード（全画面ステップ表示・スワイプ操作）

### データ

- SQLite ローカル DB（Drizzle ORM）
- シードデータ 6 件のレシピ（`mockup/app-mockup.jsx` の RECIPES を流用）
- サーバー通信なし（ローカルのみ）

### AgentBridge

- `AgentBridge` と `HookLogger` の基盤実装（A1〜A8 は stub でよい）
- フックは全発火。ログファイル書き込みを動作確認する

### 非対象（v0.1 では実装しない）

- URL 取り込み・OCR・サーバー同期・家族招待・認証

---

## 9. パフォーマンス目標（`docs/品質基準.md` §3 参照）

- コールドスタート: < 3.0 秒（iPhone SE 第3世代 / Pixel 6a 基準）
- レシピ一覧 300 件表示: < 500 ms
- FTS 検索: < 200 ms
- JS フレームレート（スクロール中）: ≥ 55 fps
