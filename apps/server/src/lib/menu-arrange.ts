...
/**
 * why の禁止パターン（正規表現・ja/en 両方）。プロンプトの指示（§10.10.2 の
 * 「してはならないこと」）を**信用しない側の決定的な防御**——評価 2 周目
 * （docs/eval/menu-rank/2026-08-29-gemini-2.5-flash.md）で「週末」への言及が
 * プロンプト遵守だけでは 0 件にならなかったため追加した。
 * 評価ハーネス（`scripts/menu-eval.ts`）はこれを import し、生成直後（ストリップ前）の
 * 値と、ここで実際にストリップされた後（表示値）の両方を測る——二重に定義しない。
 */
 ...
 *    **`BANNED_WHY_PATTERNS` に掛かる why は行ごと捨てず、why だけ undefined に
 *    落とす**（プロンプトが破られても表示は必ず守る。M1 の機械的理由への
 *    差し替えは表示側の仕事——§10.10.7-9）
