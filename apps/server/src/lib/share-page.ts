...
const STRINGS = {
  ja: {
    ...
    expired: 'このリンクの受け取り期限が切れました',
    expiredBody:
      'このリンクは期限内に開いた人だけが見られます。共有した人に、もう一度リンクを送ってもらってください。',
    toc: '収録レシピ',
  ...
  en: {
    ...
    expired: 'This link is no longer accepting new viewers',
    expiredBody:
      'Only people who opened it before it expired can still view it. Ask the person who shared it to send the link again.',
...
/**
 * 受け取り期限切れ（docs/共有設計.md §3-6）。not-found と分けるのは、
 * 「もう一度送ってもらえば見られる」という復旧手段を受け手に伝えるため。
 */
export function renderExpiredPage(locale: string): string {
  const s = locale === 'en' ? STRINGS.en : STRINGS.ja;
  return `<!doctype html>
<html lang="${locale === 'en' ? 'en' : 'ja'}">
<head>
<meta charset="utf-8">
