...
const SYSTEM_PROMPT_V1 = [
  ...
  '',
  '## 手順1〜4: レシピ化できる場合',
  '0. 写真に複数の料理・小鉢・副菜が写っている場合は、**主となる1品だけ**をレシピ化する。',
  '   主となる1品とは、最も大きく写っているもの、または中央にあるものを指す。',
  '   複数品をまとめて1つのレシピにしてはならない（材料も手順も混ざって作れなくなる）。',
  '   付け合わせ（ご飯・味噌汁・サラダ・漬物など）は材料に含めない。',
  '1. 料理を特定する（名前・ジャンル・地域）。補足テキストに店名やメニュー名があれば、',
  '   写真よりも優先してここで使う（料理の特定において最も強い手がかりであるため）。',
  '   一般に通用する料理名があるなら、説明的な言い換え（「豚バラ肉の甘辛炒め」等）ではなく',
  '   その料理名（「生姜焼き」等）を使う。',
  '2. その料理の標準的な作り方を想起する。',
...
export function buildSystemPrompt(
  ...
  unitSystem: OutputUnitSystem = DEFAULT_UNIT_SYSTEM,
): string {
  const base = variant === 'v1' ? SYSTEM_PROMPT_V1 : SYSTEM_PROMPT_V0;
  // 単位の指示は言語の指示より後ろ（言語側にも単位の言及があるため）
  return withUnitSystem(withOutputLanguage(base, outputLocale), unitSystem);
}
...
export class GeminiVisionRecipeProvider implements VisionRecipeProvider {
  ...
  async infer(input: VisionRecipeInput): Promise<VisionRecipeRaw> {
    ...
    const body = {
      systemInstruction: {
        parts: [{ text: buildSystemPrompt(this.variant, input.outputLocale, input.unitSystem) }],
      },
      contents: [
        {
          role: 'user',
