...
| 03   | Agent 用の core task scripts 整備                   | In progress | `scripts/agent/preflight.mjs`, `validate-changed-slice.mjs`, `triage-e2e-report.mjs`, `scaffold-feature-plan.mjs` | device-health 専用スクリプト（slice-test 精緻化・play-signing check は完了）                                  |
...
1. Android loop に device health と failure signal を明示的に組み込む（Step 06）。
...
1. Android failure signal の構造化 signal 定義と retry policy を実装する（Step 06）。
2. custom agents を最小構成で追加する（Step 05）。
