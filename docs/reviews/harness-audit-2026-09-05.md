...
| ------ | ------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------- |
| 🔴 高  | Security Guardrails | `issue-groomer` が `mcp__github__issue_write` を持つ。本文は「クローズしない・既定は提案まで」、§7-4 は「読むだけの役」に分類。書けるツールを持つ読み取り役はこの 1 体だけ | `issue_write` を外し、ラベル適用は司令塔が行う                                                                      | **済**。frontmatter・本文・§7-1 の行を「提案まで」に統一                  |
...
| 🟢 低  | Cost Efficiency     | 11 体すべて `model:` 未指定。毎セッション起動の PjM は軽量モデルで足りる可能性（推測）                                                                                     | `project-manager` に `model: sonnet` を付けて数セッション比較                                                       | **済**（試行。§7-5 に「数回回して判断」と明記）                           |
