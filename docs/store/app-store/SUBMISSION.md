...
| App 名（1.13.0 以降）       | 「だいどこ - 献立・レシピ管理・買い物リスト・在庫」（決定変更 B。2026-09-03 公開を ASC メールで確認） |
...
| 面        | 状態                                                         |
| --------- | ------------------------------------------------------------ |
| App Store | **1.11.0 `READY_FOR_SALE`**（2026-08-24）。1.10.2 も公開済み |
| Play      | **production `completed` / versionCode 10028 / 全量配信**    |
| build     | 10028 が `VALID`（ASC）                                      |
| whatsNew  | ja 設定済み                                                  |

> **以下は 2026-08-19 時点の記録で、すでに古い。** 経緯として残すが、
> **状態の判断には使わないこと。** 実際の状態は API で読む
> （`scripts/release/lib/play-api.mjs` / ASC は `eas.json` の `submit.production.ios` の鍵）。
> 2026-08-26 に、この節を現状と誤読して「iOS は未提出」と 2 度報告した。
> **ドキュメントは実態の記録であって実態ではない。**

## 【履歴】現在地（2026-08-19）— Android が先行、iOS は審査待ちで停止中

| 面                                    | 状態                                                                     |
| ------------------------------------- | ------------------------------------------------------------------------ |
| App Store                             | **1.9.0 (build 10024) が `WAITING_FOR_REVIEW`**（初回審査）              |
| TestFlight 内部「身内テスト」         | 本人 1 人。10024 / 10025 とも配信済み                                    |
| TestFlight 外部「身内テスト（外部）」 | 1 人（`INVITED`）。**build 10025 はベータ審査通過（`IN_BETA_TESTING`）** |
| `app.json` の `ios.buildNumber`       | **10025 で据え置き**（Android は 10026 まで進行）                        |

Android だけ 1.10.0 → 1.10.1 と進めた。**審査中のバージョンに触らない**ためで、
iOS は意図的にビルドも提出もしていない。

### 審査中に触ってよい境界（実地で確認）

- **TestFlight へのアップロードは審査に影響しない。** `eas submit -p ios` でビルドを上げても、
  `appStoreVersions` に紐付けなければ審査中のバージョンは無傷（10025 を上げた後も
  1.9.0 は `WAITING_FOR_REVIEW` のままだった）
- **ベータ審査は App Store 審査と別枠。** 外部テスト用の `betaAppReviewSubmissions` を出しても
  審査中のバージョンは動かない
- **触ってはいけないのは `appStoreVersions`**（versionString の書き換え・build の付け替え・再提出）。
  1.8.0 → 1.9.0 の差し替えはこれを使ったので、**審査開始前だから成立した**

### 外部テストを動かすのに要ったもの

内部テスターは ASC ユーザー本人なので審査なしで即配信。**外部はビルドごとに Apple のベータ審査**が要る。
だいどこ側は**ベータ App 審査情報が空**で提出できず、さいえん手帳（同じ ASC アカウント）から
連絡先を写して埋めた。`betaAppLocalizations`（ja の説明・フィードバック先）も新規作成。
**ASC の画面には「ベータ版審査に提出」ボタンが見当たらず**、`betaAppReviewSubmissions` を API から叩いた。

### 審査が通ったあとの手順

1. Android に追いついた版（1.10.x）を iOS でも出すか判断する
2. 出すなら `ios.buildNumber` を上げて EAS iOS ビルド → **`appStoreVersions` を新規作成**して提出
3. 写真取り込みのエラー表示修正（#189）は **Android の 1.10.1 にしか入っていない**。iOS に届くのはこのタイミング
