...
| POST     | `/api/v1/infer/menu-recipes` | 献立の不足日数 n（1〜7）＋手持ちタイトル（≤30）＋在庫名（≤50）＋嗜好メモ（≤400 字）→ n 品のレシピ下書きを **1 回の LLM 呼び出しで**一括生成（M3-3 の原価根拠）。認可・枠は `/infer/menu` と同じ（`x-device-id` 必須・月次枠→共有プールの順・`x-quota-source` で月次枠だけ飛ばす）。契約の正は `packages/shared/src/types/menu-recipes.ts`（`docs/買い物リスト・在庫設計.md` §10.12） |
