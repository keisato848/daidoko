...
`/infer/menu` と `/infer/menu-recipes` は他の `/infer/*` と 2 点だけ違う: ヘッダ `x-device-id`（端末の自己申告 UUID・8〜64 字 `[A-Za-z0-9_-]`）が必須で、月次の無料枠（`lib/quota-store.ts`・env `INFER_MONTHLY_FREE_LIMIT`＝既定 5・`INFER_QUOTA_DB_PATH` の SQLite で端末単位に管理）を `checkRateLimit` より先にチェックする。ヘッダ `x-quota-source: token|premium` を送ると月次枠だけ飛ばす（共有プールは通常どおり）。
