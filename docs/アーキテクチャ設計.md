...
| OCR テキストの構造化             | クライアント（v1.0） | ルールベースパーサー  | 速度重視・主要ケースをカバー           |
| OCR テキストの構造化（精度向上） | サーバー（v1.1〜）   | フロンティア AI API   | 手書き・崩れたレイアウトへの対応       |
| レシピ取り込みの補助             | サーバー（将来）     | フロンティア AI API   | 非構造化テキストの解釈                 |

### 7.2 サーバーサイド AI の方針

オンデバイス LLM（Gemma 等）はサーバーサイドでは採用しない。精度・機能更新のしやすさから、**フロンティア AI（Gemini Flash / Claude Haiku 等）の API を呼び出す**方針とする。

- コスト管理: OCR 構造化は1リクエストあたり入力テキストが小さいため Haiku / Flash クラスで十分
- プライバシー: レシピテキストのみ送信。写真・個人情報は送らない
- フォールバック: AI API が失敗した場合はルールベースパーサーの結果をそのまま返す
