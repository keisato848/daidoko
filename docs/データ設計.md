...
Recipe {
  ...
  pinnedAt        TEXT        nullable      -- 作りたいリスト（ピン留め日時, v8）。
                                            -- null = 未ピン。ホーム S01 に新しい順で表示
  placeName       TEXT        nullable      -- お店の名前（v12）。**表示は常にこちら**
  aiGenerated     INTEGER     nullable      -- 中身を AI が推定したレシピか（v17・#266）。
                                            -- 1 = AI 由来 / NULL = **不明**（「AI ではない」ではない）。
                                            -- レシピ単位・一度立てたら消さない（編集でも落とさない）
...
ピン留め（作りたいリスト）はレシピ単位・端末ローカルの状態で、版管理・共有の対象外。

`aiGenerated` もレシピ単位（版管理しない）。**リビジョンに置かないのは、編集しても落とさない
印だから** — 版ごとに新しい行を作るリビジョンに置くと、引き継ぎを書き忘れた瞬間に消える
（#220 と同型）。`NULL` は不明を表すので、**`NULL` を根拠に「人が書きました」と断言する表示は
作らない**。v17 の移行で遡れるのは `sources` に記録が残る写真・紙面 OCR 由来だけで、
相談・貼り付け・手入力・refine 由来は `NULL` のまま残る。
判断の全文は `docs/レシピ表紙AI生成設計.md` §4-1。
追加カラムは `migrate.ts` の ADD_COLUMN_MIGRATIONS（ALTER TABLE 追記方式）と
`backup.service.ts` の BACKUP_TABLES に両方登録する（旧バックアップの復元は不足カラム=null で互換）。
...
CookingLog {
  ...
  kind            TEXT        NOT NULL DEFAULT 'cooked'
                                                -- v9: 'cooked'=家で作った / 'eaten_out'=店で食べた
  placeName       TEXT        nullable          -- v9: 店名。**表示には使わない**（v12 以降）
}
```

**`kind` / `placeName`（v9, 2026-08-03）**: 「店で食べた」も「家で作った」も _その料理を体験した記録_ であり、
ホームのタイムラインは一本のままにしたい。別テーブルにすると一覧の結合・バックアップ・将来の同期が
すべて二重になるため、**1 テーブル 1 列で区別する**（`docs/お店の味を再現設計.md` §3）。
写真からレシピを作るとき「お店で食べた」を選ぶと、そのレシピは `recipes.pinnedAt` が立ち
「再現したい」棚に自動で入る。追加カラムは `migrate.ts` の `ADD_COLUMN_MIGRATIONS` と
`backup.service.ts` の `BACKUP_TABLES` の両方に登録済み。

**店名の置き場所を記録からレシピへ移した（v12, 2026-08-18）**: 店名は当初 `CookingLog` にしか無く、
写真からレシピを作る**初回にしか入力できなかった**。後から思い出して入力しても、
既にある記録は空のままなので**表示が直らない**（記録は「その日どこで食べたか」の事実で、
遡って書き換わるものではない）。「あの店のあの料理を再現する」単位はレシピ 1 件なので、
**`Recipe.placeName` を正とし、ホームのタイムライン・レシピ詳細などの表示はすべてそちらを見る**。
`CookingLog.placeName` は履歴として残すが表示には使わない。移行時に、記録に入っている店名を
レシピへ引き継ぐ（同一レシピに複数あれば**最初に入力されたもの**。空で上書きしない・
既にレシピ側に値があれば触らない）。入力欄はレシピ編集フォームに常設で、
写真取り込み画面の店名欄は**そちらへ一本化した**（同じ画面に 2 つ出さない）。
...
ShoppingItem {
  ...
  storeGroup      TEXT        nullable      -- 買う場所（v13）。例: スーパー
                                    -- **絞り込み専用**。消し込みの照合には使わない
  sortOrder       INTEGER     NOT NULL
...
## 追補: 写真パスの保存形式（v10, 2026-08-13）

**写真のパスは `FileSystem.documentDirectory` からの相対パスで保存する**
（`recipes.cover_photo_path` / `steps.photo_path` / `cooking_photos.local_path`）。

以前は絶対パスをそのまま入れていたが、**iOS の documentDirectory は
`file:///var/mobile/Containers/Data/Application/<UUID>/Documents/` で、<UUID> は
再インストール・アプリ更新・バックアップ復元で変わる**。古い絶対パスを描画すると
`<Image>` は**エラーも出さずに何も描かず**、絵文字のフォールバックは「パスが null の
とき」しか出ないため**完全な空白**になる（App Store 用スクショの撮影で発覚）。
Android の `/data/user/0/<package>/files/` は不変なので、この問題は iOS だけで起きた。

- 変換は `src/services/photo-path.ts`（`toStoredPhotoPath` / `resolvePhotoUri`）
- **読み取り時に古い絶対パスも現在の documentDirectory へ貼り替える**ので自己修復する
- 起動時に `normalizePhotoPaths()` が保存値も相対へ揃える（冪等）
- `content://` などアプリ管理外の URI は素通しする（貼り替えると壊れるため）

## 追補: レシピ帖（v11, S4 — docs/Web共有設計.md §7）

- `recipe_books`: 帖のローカル実体（title・共有状態 share_slug/share_url/share_delete_token/
  shared_at/share_locale・公開の強度 share_passcode/share_expires_at・is_legacy_share）
- `recipe_book_items`: 収録レシピ（book_id, recipe_id, position。UNIQUE(book_id, recipe_id)）
- マイグレーション v11（CREATE TABLE IF NOT EXISTS）。バックアップ対象に両テーブルを追加済み
  （共有トークンごと復元 — 別端末に復元しても共有を止められるように）
