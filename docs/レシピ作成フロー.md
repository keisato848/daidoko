...
> **作成と更新で入力の意味が違う（2026-08-27）。**
>
> - **作成（`createRecipe`）は全欄を指定する。** 渡さなかった欄は無しになる
> - **更新（`updateRecipe`）は `undefined` = 触らない / `null`・空文字 = 消す。**
>   渡されなかった欄は現行値を引き継ぐ
>
> 更新が全置換だった頃、呼び出し元 2 つ（編集画面・お店の味に近づける）が**両方とも**
> `titleReading` と `prepTimeMin` を渡しておらず、**レシピを開いて更新しただけで
> 読みがなが消えていた**（#220）。「お店の味に近づける」は `placeName` も落としていて、
> 主役機能を通すたびに店名が失われていた。2026-05-08 から 1.11.x まで出荷され続けている。
>
> 画面ごとに塞ぐと呼び出し元が増えるたび同じ穴が開くので、規則は
> `apps/mobile/src/services/recipe-update.ts` の `resolveRecipeUpdate`（純関数）に置き、
> SQLite 実装と web モックの**両方**がそれを呼ぶ。テストもそこへ直接当てる
> — 手写しで二重化すると、jest は mock 側しか通らないので**実装だけ壊れても緑になる**。
>
> 呼び出し側の約束: **自分が持っている欄は必ず渡す**（消したいなら `null` か空文字）。
> 持っていない欄は渡さない。

> ※ 以下の `saveRecipe()` は設計当時の形。実装は `createRecipe` / `updateRecipe` に
> 分かれており、`familyId` は取らず、`coverPhotoPath` / `placeName` が増えている。
...
**紙面の書式に合わせる（2026-08-23・AQUOS で食品パッケージの裏を撮って発覚）**:
本や袋の書式は**空白で区切られていない**。以下を扱えないと材料が 1 つも取れない。

- **見出しに人数が付く**（「材料(2人前)」「作り方 4人分」）。`材料` だけの行を要求していたため、
  見出しとして認識されず材料セクションに入れなかった。見出し判定の前に人数を切り出す
- **リーダー（点線）が区切り**（「じゃがいも(くし形切り)……中2個(300g)」）。空白が無いので
  行まるごとが材料名になっていた。`…` `‥` は 1 つでも、点と中黒は 2 つ以上を区切りとみなす
  （「塩・こしょう」の並列や文末の句点を割らないため）。
  **OCR は点線をピリオド 1 個に潰すことがある**（AQUOS + ML Kit で実測）ので、
  「行に空白が無い」かつ「点の前が数字でない」ときに限り、点 1 つでも区切りとして扱う
  （空白区切りの行と小数「大さじ1.5」を割らないための条件）
- **読めた項目は保存スキーマの上限に刈り込む**（料理名 100 / 材料名 50 / 分量 30 / 手順 500、
  人数 1〜99・時間 1〜999）。OCR は原材料表示や賞味期限まで拾うので、1 行でも超えると保存で詰まる

**補正 provider の失敗は画面に出さない**: `parseRecipeTextWithAssistance()` が返す
`assistanceFailures` は**開発者向け**（`__DEV__` の console にだけ出す）。`warnings` に混ぜていたため、
結果画面の帯に「gemma-native: unavailable / local-heuristic: Local heuristic could not produce
parser-friendly text」と英語の内部メッセージが並んでいた。読み取りの出来は同じ帯の
「読み取り精度」が伝えているので、利用者にはそれで足りる。**`warnings` は日本語で意味のあるものだけ。**

**確認画面へ進む条件は保存スキーマではない**: parser は読めなかった項目に編集用の空行を残すので、
材料が取れなかった下書きは `recipeFormSchema` を通らない。それを条件にしていたため、
手順が 3 つ読めていても「レシピとして必要な項目を読み取れませんでした」で行き止まりになっていた。
`ocr.agent` の `hasUsableDraft()` で「**材料か手順がどちらか読めたか**」を見る
（料理名だけでは通さない — 文章を撮っただけでも 1 行目は料理名になるため）。

**見出しに付いた人数は分ける（2026-08-23）**: OCR や手書きの見出しは「肉じゃが（2人分）」の形が多い。
そのまま料理名にすると、人数欄が空のまま料理名に「(2人分)」が残る（Pixel 9a の OCR 結果で確認）。
parser は見出し行の**末尾**に付いた「（N人分）」「(N人前)」「 N人分」だけを人数として切り出し、
本文中の人数表記には触らない。「材料: 2人分」のようなラベル付きの行は見出しにしない。
...
### 4.6 端末内 OCR から AI 読み取りへ（2026-08-23）

**入口の中身を差し替えた。** 端末内 ML Kit は版面が少しでも複雑だと成立しないことが
実測で確定したため（`docs/レシピ推論の評価設計.md` §10）、`POST /api/v1/infer/recipe-page`
に寄せた。ルート名（`import-ocr`）と利用者に見える名前（「文字入り画像から作成」）は据え置き。

|              | 端末内 OCR（旧）    | AI 読み取り（新）            |
| ------------ | ------------------- | ---------------------------- |
| 版面の複雑さ | 単純な 1 段組みのみ | 段組み・回り込み・手書きも   |
| 費用         | 無料                | **無料枠 1 回 or 広告**      |
| オフライン   | 動く                | **動かない**                 |
| 対応 OS      | Android のみ        | **iOS でも**                 |
| 枚数         | 1 枚                | **1〜4 枚を 1 つのレシピに** |

**必ず守ること:**

- **送信先の開示を出す**（`recipeImport.page.disclosure`・A 階層）。端末内処理のときは
  不要だったが、写真が端末から出るようになったので必須。**撮る前に見えている**位置に置く
- **`ensureInferenceCredit()` を通す**。無料だった機能が枠を消費するようになった
- プロンプトは**サーバーとアプリの写し 2 つ**（`apps/server/src/lib/recipe-page.ts` と
  `apps/mobile/src/services/recipe-page.provider.ts`）。BYOK はサーバーを通らないため。
  **片方だけ直すと BYOK の利用者だけ挙動が変わる**

**端末内 OCR のコードは残す。** レシート読み取り（`app/(tabs)/receipt.tsx`）が
使い続けている。レシートは 1 段組みなので ML Kit で足りており、しかも
**文字だけを送れば写真を送らずに済む**（`docs/在庫・レシート設計レビュー.md` §3.4）。
...
stateDiagram-v2
    ...
    ImageProcessing --> TextSuccess : 画像内テキスト OCR 成功
    ImageProcessing --> LabelFailed : provider 未設定/画像解析失敗
    TextSuccess --> RecipeTextParse : OCR テキスト → 共通 parser
    RecipeTextParse --> Review : 読み取った内容で入力フォームを初期化
...
同期経路はクライアント内に限定する。Android は ML Kit Image Labeling と ML Kit Text Recognition、iOS は Apple Vision または同等のオンデバイス provider を使用する。サーバー AI / サーバー画像解析 fallback は入れない。

料理だけが写った写真では、画像ラベルから低信頼の下書きを生成する。料理写真の中にメニューカード、手書きメモ、レシピ本文などの文字が読める場合は、OCR raw text を既存 `parseOcrText()` / `parseRecipeTextWithAssistance()` に渡し、読み取った料理名・材料・手順を `RecipeForm` の初期値へ反映する。つまり「写真から 100 点の完成レシピを自動保存する」のではなく、写真から得られた根拠で入力画面を埋めた状態に遷移させる。
...
async function runPhotoRecipePipeline(input: {
  imageUri: string;
  labelProvider: ClientImageLabelProvider;
  ocrProvider?: ClientOcrProvider;
}) {
  const processed = await preprocessImageForOcr(input.imageUri);
  const labels = await input.labelProvider.labelImage(processed.imageUri);

  const recognized = input.ocrProvider
    ? await input.ocrProvider.recognize(processed.imageUri)
    : undefined;
  if (recognized && hasEnoughOcrText(recognized.rawText)) {
    const parsed = await parseOcrText(recognized.rawText);
    return { draft: parsed.formData, confidence: parsed.confidence, warnings: parsed.warnings };
  }
...
| OCR      | `src/services/client-ocr.provider.ts`, `src/services/ocr.service.ts` | 画像内文字が読める場合に共通 parser へ渡し、フォーム初期値へ反映              |
...
| IMG-RECIPE-REQ-07 | 画像内文字が読める場合は読み取った内容を入力フォームに反映する    | `recipe-photo.agent.ts`, `client-ocr.provider.ts`              | `IMG-RECIPE-AGT-01`, `IMG-RECIPE-E2E-04` |
| IMG-RECIPE-REQ-08 | 100 枚以上の同梱生成画像で保存可能な下書き作成を一括検証する      | `photo-recipe-batch-fixtures.ts`, `import-photo.tsx`           | `IMG-RECIPE-E2E-04`                      |
...
## 9. 連続撮影（撮影系フロー共通・2026-09-05 オーナー要望）

「写真を撮るパターンのとき、連続して撮影できるように」への共通実装。

**ループは `src/services/photo-capture.service.ts` の `capturePhotoSeries()` 1 箇所** —
各画面は残り枠（上限 − 取り込み済み枚数）を渡すだけで、ループを持たない。
`launchCameraAsync` は 1 回 1 枚しか撮れないため、撮影成功のたびに共通ダイアログ
`confirmContinueCapture`（「続けて撮る（あと n 枚）/ これで完了」）を挟んで上限まで回す。

- 上限 1 の呼び出しは従来どおり**確認なし**で 1 枚
- 1 枚目のキャンセル → 空配列（画面はキャンセル扱い）
- 2 枚目以降のキャンセル → **撮った分は生かして**打ち切り
- ギャラリーは system Photo Picker の複数選択（`pickManyFromGallery` +
  `selectionLimit`・Android 13+/iOS 14+ 対応・expo-image-picker の
  `allowsMultipleSelection`）に委ねる — 連続ダイアログは出さない
- 取り込んだ写真はどの画面でも**従来と同じ後段**（前処理・縮小・保存経路）に流れる。
  取得だけを差し替えているので、送信前の縮小（§冷蔵庫写真設計 §8 の教訓）はそのまま効く

### 対象画面と上限（サーバー契約に合わせる）

| 画面                       | 上限                                 | 根拠                                                  |
| -------------------------- | ------------------------------------ | ----------------------------------------------------- |
| 文字入り画像（import-ocr） | `MAX_RECIPE_PAGE_IMAGES` = 4         | /infer/recipe-page（紙面は表・裏で 1 レシピ）         |
| 冷蔵庫写真（fridge）       | `MAX_FRIDGE_IMAGES` = 2              | /infer/fridge（冷蔵室＋野菜室。2 枚で 1 推論 1 消費） |
| AI 相談（consult）         | `MAX_CONSULT_IMAGES_PER_MESSAGE` = 2 | 1 メッセージあたり                                    |
| 調理記録（log）            | `MAX_COOKING_LOG_PHOTOS` = 6         | ローカル保存の既存上限                                |

### スコープ外（変更しない・理由つき）

- **写真からレシピ（import-photo）と食事写真（consume-meal）**: サーバー契約が
  `imageBase64` **単数**の 1 枚設計（/infer/photo・/infer/meal）。複数枚化は契約・
  プロンプト・コストの再設計が要るため今回はやらない
- **感想調整（refine）**: 撮影で取り込むのは「作った結果（cooked）」の 1 枚だけで、
  2 枚目（target）は既存写真から選ぶ設計。撮影上限が 1 なので連続撮影の対象外
  （上限 1 は確認なし、の規則がそのまま当てはまる）
