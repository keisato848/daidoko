...
レシピ作成には4つの入力方式がある。いずれも最終的に同一の `saveRecipe()` ロジックを通じて `Recipe` + `RecipeRevision` を生成する。
...
OCR で得られた生テキスト、および S10a で貼り付けられた自由文を材料・手順・タイトルに分類するロジック。
...
S10a の初期実装では、上記方針を端末内のルールベース parser として実装する。`材料` / `作り方` 見出し、分量付き行、番号付き手順を優先して分類し、解析後は必ず `RecipeForm` で確認・編集してから保存する。
