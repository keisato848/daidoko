/**
 * 料理の出自（cuisine）の推定と、盛り付けの 1 行（`docs/レシピ表紙AI生成設計.md` §2-1 の
 * 「プロンプト（C-1・2026-09-13）」）。
 *
 * **ここが正。** サーバーは実行時に `@daidoko/shared` を取り込まない方針
 * （tsconfig の rootDir が src に閉じている）ため `apps/server/src/lib/cuisine.ts` が
 * 同じ内容の写しを持ち、`__tests__/shared-parity.test.ts` が突合する。
 * モバイル（BYOK）はこのファイルを直接 import する。**片方だけ直さないこと。**
 *
 * ## なぜ locale ではなく出自か
 *
 * 従来は盛り付けの 1 行を UI 言語（locale）で決めていた。だが UI が日本語でも
 * 「ボロネーゼ」は日本の食卓の盛り付けではないし、UI が英語でも「肉じゃが」は
 * 和食器に盛るのが自然。料理名・タグ・材料名から出自を推定して盛り付けを決める。
 *
 * **判定不能なら null を返す**（呼び出し側が現行の locale 行へフォールバックする）。
 * 推定を外すより「今より悪くならない」ことを優先する — 栓は server 側の env
 * `COVER_IMAGE_CUISINE_HINT`（既定 off）で、評価で現行以上と確認してから on にする。
 */
import { vocabKey } from './ai';
