 ...
 *
 * **数量は「写真から数えられる・読み取れる場合のみ」**（オーナー決定 2026-09-05 —
 * 当初の「欄ごと持たせない」を上書き）。自由テキスト（「3本」「1パック」「約200g」）で、
 * 数えられないものは欠落/null。**推測で埋めない**のは従来どおり — 推測した数字は在庫で
 * 合算されて「家に無いものが在庫にある」状態を静かに作る。確認シートで必ず編集できる。
 *
...
/** 数量テキストの上限（自由テキスト。「3本」「約200g」等）。 */
export const MAX_FRIDGE_QUANTITY_LENGTH = 30;
...
export const fridgeItemSchema = z.object({
  ...
  /**
   * 数量（自由テキスト・**写真から数えられた場合のみ**）。省略/null = 読めなかった。
   * **省略可なのが互換の要** — 旧アプリ×新サーバー（未知フィールドは無視）・
   * 新アプリ×旧サーバー（フィールドが来ない）のどちらも壊れない。
   */
  quantity: z.string().min(1).max(MAX_FRIDGE_QUANTITY_LENGTH).nullable().optional(),
});
